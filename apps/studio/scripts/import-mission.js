...
async function run() {
  const file = path.join(__dirname, "../seed/mission.json");
  const raw = fs.readFileSync(file, "utf-8");
  const doc = JSON.parse(raw);

  const existing = await client.fetch('*[_type == "page" && slug.current == "mission"][0] { _id }');

  if (existing && existing._id) {
    await client.patch(existing._id).set(doc).commit();
    console.log("Updated existing mission page:", existing._id);
  } else {
    const created = await client.create(doc);
    console.log("Created new mission page:", created._id);
  }
}
